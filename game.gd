...
const TRAIN_TIME := 16.0      # seconds of self-training for +1 max heart
const TEACH_TIME := 20.0      # seconds in the clan hall to summon your clan
...
func _pick_key_spot(home: Dictionary, min_from_home: float, used: Array) -> Vector3:
	# never inside a walled room (unreachable) and spread keys apart
	var spot := random_spot()
	for i in 40:
		var s := random_spot()
		if _near_any_room(s):
		...
		if used.any(func(u): return Vector2(u.x - s.x, u.z - s.z).length() < 16.0):
			continue
		spot = s
		if Vector2(s.x - home["x"], s.z - home["z"]).length() > min_from_home:
			break
	return spot

# One key per element. The player's own key sits a real trek from home; the two
# rival keys sit elsewhere (colour-coded, but the player can't take them).
func _spawn_keys() -> void:
	_free_keys()
	var home: Dictionary = cave_by_owner.get(player.el, { "x": 0.0, "z": 0.0 })
	var used: Array = []
	for el in ["fire", "water", "grass"]:
		var spot := _pick_key_spot(home, (55.0 if el == player.el else 0.0), used)
		used.append(spot)
